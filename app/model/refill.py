__author__ = "Hatam"
__copyright__ = "Copyright 2023"
__credits__ = []
__license__ = "MIT"
__version__ = "1.0.0"
__maintainer__ = ""
__email__ = "hatamabolghasemi@gmail.com"
__status__ = "Production"

from datetime import datetime
from utils.my_dotenv import MyDotenv
from app.db.base import Base

de = MyDotenv()


class Refill(Base):
    def __init__(
        self,
        refill_id: int = -1,
        doctor_id: int = -1,
        patient_id: int = -1,
        medication_name: str = "",
        prescription_details: str = "",
        time: str = datetime.now().strftime(de.dotenv_values.get("TIME_FORMAT")),
    ):
        super().__init__()
        self._refill_id = refill_id
        self._doctor_id = doctor_id
        self._patient_id = patient_id
        self._medication_name = medication_name
        self._prescription_details = prescription_details
        self._time = time

    @property
    def refill_id(self):
        return self._refill_id

    @refill_id.setter
    def refill_id(self, a):
        self._refill_id = a

    @property
    def doctor_id(self):
        return self._doctor_id

    @doctor_id.setter
    def doctor_id(self, a):
        self._doctor_id = a

    @property
    def patient_id(self):
        return self._patient_id

    @patient_id.setter
    def patient_id(self, a):
        self._patient_id = a

    @property
    def medication_name(self):
        return self._medication_name

    @medication_name.setter
    def medication_name(self, a):
        self._medication_name = a

    @property
    def prescription_details(self):
        return self._prescription_details

    @prescription_details.setter
    def prescription_details(self, a):
        self._prescription_details = a

    @property
    def time(self):
        return self._time

    @time.setter
    def time(self, a):
        self._time = a

    def add(self) -> int:
        """
        this method add refill model to database
        :return:
        """
        try:
            record = self.my_db[Refill.__name__].insert_one(
                {
                    "doctor_id": self.doctor_id,
                    "patient_id": self.patient_id,
                    "medication_name": self.medication_name,
                    "prescription_details": self.prescription_details,
                }
            )
            self.refill_id = record.inserted_id
            return self.refill_id
        except Exception as e:
            self.log.error(e)
            return -1
