...
class Parser(object):
    ...
    def action_steps(self, line):
        ...
        # if line.startswith("|"):
        if line.startswith("|") or os.path.isfile(line): # added
    ...
    def action_table(self, line):
        ...
        # added
        # first: pip install pandas openpyxl
        if line.startswith("|"):
            ...
            if file_extension == ".xlsx":
                df = pd.read_excel(line, dtype=str)
            elif file_extension == ".csv":
                df = pd.read_csv(line, dtype=str)
            self.table = model.Table(df.columns.values.tolist(), self.line, df.values.tolist())
        else:
            if self.examples:
